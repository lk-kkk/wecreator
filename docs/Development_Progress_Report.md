...
## 2026-04-18 PRD功能缺口修复 (commit `e6229e3`)

### 修复范围
对照PRD审查发现的3个功能缺口，全部P0+P1项完成修复。

### 功能4 — 任务附件上传（后端+前端）
**后端新增：**
- `dto/index.ts`: 新增 `AddAttachmentDto` (fileName/fileUrl/fileSize/fileType)
- `task.service.ts`: 新增 `addAttachment` / `deleteAttachment` / `getAttachments` 三个方法，附件上限10个
- `task.service.ts`: `getTaskDetail` include attachments，返回值加 `attachments[]`
- `task.controller.ts`: 新增三个路由 `GET/POST /tasks/:id/attachments` + `DELETE /tasks/:id/attachments/:attachmentId`

**前端新增：**
- `TaskCreatePage.vue` Step1：拖拽+点击上传区域，支持所有文档/图片格式，≤50MB/个，最多10个
- 文件列表展示：文件名+大小+类型图标+状态标签+删除按钮
- 发布时自动调用 `addAttachment` 绑定附件到任务

### 功能5 — 项目清单管理增强（后端+前端）
**后端修复：**
- `CreateProjectDto`: 新增 `managerId?` 字段（可选，默认为创建者）
- `createProject`: 500项目上限校验；使用 `dto.managerId ?? userId`
- `updateProject`: 归档项目只读保护（ForbiddenException）
- 列表/详情：批量查询 `company_users` 返回 `managerName` 字段
- `serializeProject`: tasks 包含 `totalBudget` 字段
- `CreateTaskDto`: 新增 `projectId?` 字段
- `createTask`: 写入 `project_id` 字段到数据库

**前端修复：**
- `ProjectListPage.vue`: 
  - 新增筛选区（关键词/状态/负责人）
  - 表格补全「负责人」「任务数」「操作（归档）」列
  - 新建项目弹窗加「项目负责人」下拉（可选其他企业用户）
- `ProjectDetailPage.vue`: 
  - 负责人显示 `managerName`（非数字ID）
  - 关联任务区加总预算汇总
  - 加「新建关联任务」按钮（跳转发布任务并预填projectId）
- `TaskCreatePage.vue` Step1: 加「关联项目」下拉选择
  - 支持路由 `query.projectId` 预填（从项目详情页跳转自动关联）
- 新建 `api/project.ts` 项目API封装

### 功能3 — AI任务顾问（后端+前端）
**后端修复：**
- `ai.service.ts`: `chat()` 返回新增 `isComplete` 字段
  - 优先 JSON.parse 整个响应；失败则提取 markdown ```json``` 块
  - 判断条件：解析结果含 `title` 字段则认为完整

**前端新增（整个AI顾问UI）：**
- `TaskCreatePage.vue`:
  - PageHeader 右侧「🤖 AI 任务顾问」按钮（仅企业已配置LLM时显示）
  - 560px 右侧 Drawer，含智能体切换下拉（显示active智能体）
  - 消息列表（用户气泡/AI气泡），30轮限制计数显示
  - 底部输入框，Ctrl+Enter 快速发送
  - 自动检测JSON建议，显示「⚡ 一键填充到表单」区域
  - `fillFromAI()`: 回填 title/description/taskMode/startDate/endDate/roles，已有内容弹确认覆盖
  - 错误处理：429/403/504/网络异常显示具体错误消息
  - 对话超30轮禁用输入

### 接口验证结果
| 接口 | 状态 |
|------|------|
| POST /tasks/108/attachments | ✅ 200 返回attachmentId |
| DELETE /tasks/108/attachments/1 | ✅ 200 返回attachmentId |
| GET /tasks/108 | ✅ 含attachments[] |
| GET /projects | ✅ 含managerName字段 |
| POST /tasks + projectId | ✅ project_id写入DB |
...
### Git信息
- commit: `e6229e3`
- 12 files changed, 673 insertions(+), 350 deletions(-)
- 新增文件: `apps/pc-admin/src/api/project.ts`
