...
import {
  IsString, IsIn, IsOptional, Matches, MinLength, MaxLength,
...
export class CreateSubaccountDto {
  ...
  @IsString() @Matches(/^1[3-9]\d{9}$/, { message: '手机号格式不正确' }) phone: string;

  @ApiProperty({ description: '密码（8-32位，含大小写+数字）' })
  @IsString() @MinLength(8) @MaxLength(32)
  @Matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/, { message: '密码需含大小写字母和数字' })
  password: string;

  @ApiProperty({ enum: ['super_admin', 'task_admin', 'finance_admin', 'operator'], description: '角色' })
  @IsIn(['super_admin', 'task_admin', 'finance_admin', 'operator']) role: string;
}

export class UpdateSubaccountDto {
  @ApiPropertyOptional() @IsOptional() @IsString() @MaxLength(20) name?: string;

  @ApiPropertyOptional({ enum: ['super_admin', 'task_admin', 'finance_admin', 'operator'] })
  @IsOptional() @IsIn(['super_admin', 'task_admin', 'finance_admin', 'operator']) role?: string;
...
export class SubaccountService {
  ...
  // 创建子账号
  // ================================================================
  async create(companyId: number, operatorId: number, dto: CreateSubaccountDto) {
    ...
    if (count >= this.MAX_ACCOUNTS) {
      throw new BadRequestException(`子账号数量已达上限（${this.MAX_ACCOUNTS}个）`);
    }

    // 手机号唯一性
    const exists = await this.prisma.companyUser.findFirst({
      where: { companyId: BigInt(companyId), phone: { contains: dto.phone }, status: { not: 'deleted' } },
    });
    if (exists) throw new ConflictException('该手机号已被注册');

    const hash = await bcrypt.hash(dto.password, 10);
    // 手机号加密存储（与主账号保持一致，此处简化存明文供开发阶段使用）
    const account = await this.prisma.companyUser.create({
      data: {
        companyId:    BigInt(companyId),
        name:         dto.name,
        phone:        dto.phone,       // 生产环境需 AES 加密
        passwordHash: hash,
    ...
    this.logger.log(`子账号创建: company=${companyId}, id=${account.id}, role=${dto.role}`);
  ...
  // 更新子账号（角色 / 密码）
  // ================================================================
  async update(companyId: number, operatorId: number, targetId: number, dto: UpdateSubaccountDto) {
    await this._assertSuperAdmin(operatorId);
  ...
  // 启用 / 禁用
  // ================================================================
  async setStatus(companyId: number, operatorId: number, targetId: number, active: boolean) {
    await this._assertSuperAdmin(operatorId);
    const target = await this._getAccount(companyId, targetId);

    if (target.role === 'super_admin') throw new ForbiddenException('不能禁用超级管理员');
    if (Number(target.id) === operatorId) throw new BadRequestException('不能禁用自己');
  ...
  async remove(companyId: number, operatorId: number, targetId: number) {
    ...
    if (target.role === 'super_admin') throw new ForbiddenException('不能删除超级管理员');
    if (Number(target.id) === operatorId) throw new BadRequestException('不能删除自己');
  ...
  private async _assertSuperAdmin(userId: number) {
    const user = await this.prisma.companyUser.findUnique({ where: { id: BigInt(userId) } });
  ...
  private async _getAccount(companyId: number, userId: number) {
    const user = await this.prisma.companyUser.findFirst({
      where: { id: BigInt(userId), companyId: BigInt(companyId), status: { not: 'deleted' } },
    });
  ...
  private _format(a: any) {
    return {
      id:         Number(a.id),
      name:       a.name,
      phone:      a.phone ? a.phone.slice(0, 3) + '****' + a.phone.slice(-4) : '—',
      role:       a.role,
      status:     a.status,
